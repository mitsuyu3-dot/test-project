...
function useSuit(suit, x, y, size = 16, flip = false) { return `<use href="#suit-${suitName(suit)}" x="${x - size / 2}" y="${y - size / 2}" width="${size}" height="${size}" fill="${suitColor(suit)}"${flip ? ` transform="rotate(180 ${x} ${y})"` : ""} />`; }
function indexMarkup(rank, suit, x, y, rotate = false) { return `<g class="card-index"${rotate ? ` transform="rotate(180 ${x} ${y})"` : ""}><text x="${x}" y="${y}" fill="${suitColor(suit)}">${rank}</text>${useSuit(suit, x + 1, y + 8, 9)}</g>`; }
...
export function renderCardFace({ rank, suit, faceUp = true, squeezeProgress = 100 }) {
  ...
  const pips = isNumber && (faceUp || progress > 0) ? (PIP_LAYOUTS[rank] ?? []).map(pip => useSuit(suit, pip.x, pip.y, rank === "A" ? 31 : 16, pip.flip)).join("") : "";
