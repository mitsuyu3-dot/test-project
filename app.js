...
function cardMarkup(card) {
  ...
  const front = `<span>${card.rank}</span><b>${card.suit}</b>`;
  if (card.revealState === "squeezing") return `<button class="card card-back reveal-card is-squeezing" data-card-id="${card.id}" type="button" aria-label="絞り中のカード。ドラッグで公開範囲を変更"><span class="squeeze-face ${card.color === "red" ? "is-red" : ""}" style="clip-path:inset(0 ${100 - card.squeezeProgress}% 0 0)">${front}</span><span class="squeeze-mask">?</span></button>`;
  ...
  return `<button class="card ${card.color === "red" ? "is-red" : ""} is-revealed" data-card-id="${card.id}" type="button" aria-label="公開済みカード">${front}</button>`;
}
...
function revealCard(id, internal = false) { const card = findCard(id); if (!card || !card.isDealt || card.revealState === "revealed" || (game.isAnimating && !internal)) return false; card.revealState = "revealed"; card.squeezeProgress = 100; render(); return true; }
...
function startSqueeze(card) { if (!card || card.revealState === "revealed" || game.isAnimating || !["revealing", "third"].includes(game.phase)) return; card.revealState = "squeezing"; render(); }
...
let pointerCard = null; let pointerStart = 0;
document.querySelector(".hands").addEventListener("pointerdown", event => { const element = event.target.closest(".reveal-card"); if (!element) return; pointerCard = findCard(element.dataset.cardId); pointerStart = event.clientX; startSqueeze(pointerCard); element.setPointerCapture?.(event.pointerId); });
document.querySelector(".hands").addEventListener("pointermove", event => { if (!pointerCard) return; const progress = ((event.clientX - pointerStart) / 80) * 100 + pointerCard.squeezeProgress; setSqueezeProgress(pointerCard, progress); pointerStart = event.clientX; });
